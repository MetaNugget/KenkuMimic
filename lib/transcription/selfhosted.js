...
// Unlike Deepgram/AssemblyAI, the pod is billed by wall-clock regardless of
// whether a socket is idle, so there's no cost pressure to close it quickly.
// A generous window absorbs slow-inference stragglers (see README "Tuning")
// instead of racing the server's own INACTIVITY_TIMEOUT_MS and losing the
// tail of an utterance.
const TRAILING_SILENCE_MS = 10000;
...
function createSelfhostedAdapter() {
      ...
      if (status.ready) {
        ...
        if (port?.ip && port?.public) {
          if (!announcedHealthCheck) {
            ...
            onProgress?.('⚙️ GPU host up, waiting for model load...');
          }
          // A 200 here implies the model is loaded, not just that the
          // container started: main.py loads it inside FastAPI's lifespan
          // startup, which completes before uvicorn accepts connections.
          const healthy = await fetch(`http://${port.ip}:${port.public}/health`)
            .then((res) => res.ok)
            .catch(() => false);
          if (healthy) {
  ...
  async function openConnection(speakerId) {
    const token = process.env.SELFHOSTED_AUTH_TOKEN ?? '';
    const url = `ws://${serverAddress.ip}:${serverAddress.port}/transcribe?sample_rate=16000&token=${encodeURIComponent(token)}`;
  ...
  return {
    async connect(onProgress) {
      ...
      pool = createSpeakerPool({ openConnection, closeConnection }, { trailingSilenceMs: TRAILING_SILENCE_MS });
    },
